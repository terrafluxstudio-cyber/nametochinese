...
type ViName = { vietnamese: string; chinese: string; gender: string };
const ACCENT = '#2C5F8A';

export default function ViSearchClient() {
  const searchParams = useSearchParams();
  const [q, setQ] = useState('');
  const [results, setResults] = useState<ViName[]>([]);
  const [loading, setLoading] = useState(false);
  const abortRef = useRef<AbortController | null>(null);
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    fetch('/api/search-vi?q=Nguyen').catch(() => {});
  ...
  function search(query: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/search-vi?q=${encodeURIComponent(query)}`, { signal: abortRef.current.signal });
  ...
  const CHIPS = ['Nguyễn','Hồ Chí Minh','Trần','Lê','Võ','Phạm','Ngô Đình Diệm','Dương Văn Minh'];
  ...
  return (
    <>
      <SearchInput
        ...
        onSubmit={(v) => search(v)}
        placeholder="Hồ Chí Minh / Nguyễn / 胡志明"
        autoFocus
      />
      <p className="text-xs text-gray-400 mb-4 mt-3 pl-1">输入越南语姓名（含声调）或中文名</p>

      {!q && !loading && (
        <div className="mb-8">
          <p className="text-xs text-gray-400 mb-2">常见越南语名试试：</p>
        ...
        {!loading && results.map((r, i) => (
          <div key={i} className="rounded-xl px-5 py-4"
            ...
            <div className="flex items-baseline gap-3">
              <span className="text-xl font-medium" style={{ color: '#1A1A1A' }}>{r.vietnamese}</span>
      ...
      {!loading && results.length === 0 && q.length > 0 && (
        <div className="text-center mt-8">
          ...
          <p className="text-xs text-gray-400 mb-3">越南语人名多有汉越词对应汉字，搜索时建议包含声调</p>
          <a href="/naming-rules"
