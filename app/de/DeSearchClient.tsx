...
type DeName = { german: string; chinese: string; gender: string };
const ACCENT = '#2C5F8A';

export default function DeSearchClient() {
  const searchParams = useSearchParams();
  const [q, setQ] = useState('');
  const [results, setResults] = useState<DeName[]>([]);
  const [loading, setLoading] = useState(false);
  const abortRef = useRef<AbortController | null>(null);
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    fetch('/api/search-de?q=a').catch(() => {});
  ...
  function search(query: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/search-de?q=${encodeURIComponent(query)}`, { signal: abortRef.current.signal });
  ...
  const CHIPS = ['Merkel','Müller','Beethoven','Kant','Einstein','Goethe','Schiller','Bach','Brahms','Mozart','Freud','Marx'];
  ...
  return (
    <>
      <SearchInput
        ...
        onSubmit={(v) => search(v)}
        placeholder="Müller / Merkel / 贝多芬"
        autoFocus
      />
      <p className="text-xs text-gray-400 mb-4 mt-3 pl-1">输入德文姓名或中文名</p>

      {!q && !loading && (
        <div className="mb-8">
          <p className="text-xs text-gray-400 mb-2">常见德语名试试：</p>
        ...
        {!loading && results.map((r, i) => (
          <div key={i} className="rounded-xl px-5 py-4"
            ...
            <div className="flex items-baseline gap-3">
              <span className="text-xl font-medium" style={{ color: '#1A1A1A' }}>{r.german}</span>
      ...
      {!loading && results.length === 0 && q.length > 0 && (
        <div className="text-center mt-8">
          ...
          <a href={`/convert?lang=${encodeURIComponent('德語')}&q=${encodeURIComponent(q)}`}
