...
function isArabic(s: string) { return /[؀-ۿ]/.test(s); }
...
export async function GET(req: NextRequest) {
  ...
  const col = isArabic(q) ? 'arabic' : (isChinese(q) ? 'chinese' : 'arabic');
  const pattern = isChinese(q) ? `${q}%` : `%${q}%`;

  const sql = `SELECT arabic, chinese, gender FROM arabic_names
               WHERE ${col} LIKE ?
