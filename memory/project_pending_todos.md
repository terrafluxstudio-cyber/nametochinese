...
> **真问题 = 函数区域错配：** Turso 在东京，但 API 函数跑在账号默认区 iad1(美东)。大陆用户搜一次 = 大陆→美东→东京→美东→大陆，绕地球。
> **修复：** 新建 `vercel.json` `{"regions":["hnd1"]}`，把函数钉东京（Hobby 免费版只认项目级单区；路由级 `preferredRegion` 被忽略，已弃用）。**验证：x-vercel-id 由 sin1::iad1 → sin1::hnd1；ping dbMs 815→62(冷)/6-7(暖)，降 ~20 倍。**
> **仍未解（认了）：** 打开页面慢=无 ICP→无大陆 CDN 节点，静态页也得从海外边缘拉，地理问题，非 ICP 不可解。只剩压 JS 包等边角料。
> **⚠️ 别再把"提速"和"2123 页不被索引"捆一起** —— 后者是**域权重+程序化页同质**问题（页面有真实静态内容+metadata，渲染没毛病），治法=真 dofollow 外链 + 页面差异化，跟速度无关。
...
- [x] **地名音译分表**（2026-06-07 核结：code 完整，无可补）：transliterate(q,lang,type) 按 type 选 place 表已正确实现（lib/transliterate.ts:556），9 语言有 place 表全部接通。⚠️核查发现 place 表与 person 表内容几乎相同（俄语21行仅3行差，且差异只是全角/半角括号），真正差异化需官方《地名译音表》源（wiki 之外不可得，同 person 表调查结论）。故无可安全补，关闭。
...
- [x] **P2 单个含义词→中文** `/[word]-in-chinese`（2026-06-07 上线，47词）：用户选"单个含义词"（纹身/取名/艺术搜索量最大）而非 how-to-say 短语。data/english_words.json 自选高搜索词+自写厚释义（译文=事实不dump词典）。复用 /[slug] 路由派发（撞人名则人名优先）+ /words-in-chinese hub。可继续加词扩量。短语簇(how-to-say)留作后续可选
...
- [x] **/gov-titles 扩充**（2026-06-07）：555→562，补 ITU/UPU/UNWTO/WCO/ISO/IEA/BIS。核查确认其余知名国际组织（IOC/INTERPOL/ICJ/ICC/FIFA/WIPO/ILO/FAO/IAEA/UNHCR/UNICEF 等）早已覆盖，国际组织板块已近乎穷尽。各国条目细化为开放项，按需再加。
